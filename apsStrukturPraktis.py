...
# ==========================================
st.set_page_config(
    page_title="Smart_Engineer OMNI-X (Pro Report)",
    page_icon="🏗️",
    layout="wide",
...
# ==========================================
# 2. FUNGSI TEKNIS & PDF GENERATOR
# ==========================================

# --- CLASS PDF REPORT (NEW FEATURE) ---
class PDFReport(FPDF):
    def header(self):
        # Logo placeholder (opsional) atau Judul Besar
        self.set_font('Arial', 'B', 14)
        self.set_text_color(13, 71, 161) # Warna Biru
        self.cell(0, 10, 'SMART_ENGINEER OMNI-X', 0, 1, 'C')
        self.set_font('Arial', 'B', 10)
        self.set_text_color(0, 0, 0)
        self.cell(0, 5, 'LAPORAN PERHITUNGAN STRUKTUR', 0, 1, 'C')
        self.line(10, 25, 200, 25) # Garis horizontal
        self.ln(10)
    ...
    def footer(self):
        ...
        self.cell(0, 10, f'Halaman {self.page_no()} | Generated by Smart_Engineer AI | {datetime.datetime.now().strftime("%Y-%m-%d")}', 0, 0, 'C')
    ...
    def add_result_row(self, label, value):
        self.set_font('Arial', 'B', 10)
        self.cell(60, 8, label, 0)
        self.set_font('Arial', '', 10)
...
with st.sidebar:
    ...
    <div style="text-align: center; padding: 15px; border-bottom: 2px solid #0D47A1; margin-bottom: 20px;">
        ...
        <span style="color:#FF6F00 !important; font-weight:bold; letter-spacing:2px; font-size:0.8rem;">PRO REPORT VERSION</span>
    </div>
    """, unsafe_allow_html=True)
...
# ==========================================
# 4. LOGIKA PERHITUNGAN (INTEGRATED + PDF)
# ==========================================
...
# --- A. BEBAN & ATAP ---
    ...
    if st.button("HITUNG & SIMPAN DATA"):
        ...
        # --- PDF GENERATION FOR LOAD ---
        pdf = PDFReport()
        pdf.add_page()
        pdf.chapter_title("1. ANALISIS BEBAN GEMPA (WT)")
        
        # Body PDF
        text_body = f"""
        Beban Atap:
        - Luas: {A1} m2
        - Beban Mati (DL): {D1} kg/m2
        - Beban Hidup (LL): {L1} kg/m2
        
        Beban Lantai Tipikal ({N_typ} Lantai):
        - Luas: {A2} m2
        - Beban Mati (DL): {D2} kg/m2
        - Beban Hidup (LL): {L2} kg/m2
        """
        pdf.chapter_body(text_body)
        pdf.ln(5)
        
        pdf.chapter_title("HASIL PERHITUNGAN")
        pdf.add_result_row("Berat Atap (Wa)", f"{Wa:,.0f} kg")
        pdf.add_result_row("Berat Tipikal (Wtyp)", f"{Wt_typ:,.0f} kg")
        pdf.set_font('Arial', 'B', 12)
        pdf.cell(0, 10, f"TOTAL BERAT SEISMIK (Wt): {Wt_tot:,.2f} kN", 0, 1)
        
        # Convert to binary
        pdf_bytes = pdf.output(dest='S').encode('latin-1')
        st.download_button(label="📥 DOWNLOAD LAPORAN PDF", data=pdf_bytes, file_name="Laporan_Analisis_Beban.pdf", mime="application/pdf")
...
# --- B. GEMPA ---
    ...
    if st.button("HITUNG & SIMPAN V BASE"):
        ...
        if Fa is not None:
            ...
            # --- PDF GENERATION FOR GEMPA ---
            pdf = PDFReport()
            pdf.add_page()
            pdf.chapter_title("2. PARAMETER GEMPA (SNI 1726:2019)")
            
            body_txt = f"""
            Data Masukan:
            - Kelas Situs: {site_class}
            - Percepatan Batuan Dasar (Ss): {Ss} g
            - Percepatan 1 Detik (S1): {S1} g
            - Faktor R: {R_val}
            - Faktor Ie: {Ie_val}
            - Berat Seismik (Wt): {Wt} kN
            """
            pdf.chapter_body(body_txt)
            
            pdf.chapter_title("HASIL ANALISIS")
            pdf.add_result_row("Koefisien Fa", f"{Fa:.3f}")
            pdf.add_result_row("Koefisien Fv", f"{Fv:.3f}")
            pdf.add_result_row("SDS", f"{Sds:.3f} g")
            pdf.add_result_row("SD1", f"{Sd1:.3f} g")
            pdf.ln(5)
            pdf.set_font('Arial', 'B', 12)
            pdf.cell(0, 10, f"GAYA GESER DASAR (V): {V:,.0f} kN", 0, 1)
            
            pdf_bytes = pdf.output(dest='S').encode('latin-1')
            st.download_button(label="📥 DOWNLOAD LAPORAN GEMPA", data=pdf_bytes, file_name="Laporan_Gempa_SNI.pdf", mime="application/pdf")
...
# --- C. STRUKTUR ATAS ---
    ...
    if st.button("DESAIN & SIMPAN DATA MATERIAL"):
        ...
        try:
            ...
            # --- PDF GENERATION FOR BALOK ---
            pdf = PDFReport()
            pdf.add_page()
            pdf.chapter_title("10. DESAIN PENULANGAN BALOK (SNI 2847:2019)")
            
            body_txt = f"""
            Data Penampang & Material:
            - Dimensi: {b:.0f} x {h:.0f} mm
            - Mutu Beton (fc'): {fc} MPa
            - Mutu Baja (fy): {fy} MPa
            - Momen Perlu (Mu): {Mu} kNm
            - Decking: {ds} mm
            """
            pdf.chapter_body(body_txt)
            
            pdf.chapter_title("HASIL ANALISIS")
            pdf.add_result_row("Tinggi Efektif (d)", f"{d:.0f} mm")
            pdf.add_result_row("Beta 1", f"{beta1:.2f}")
            pdf.add_result_row("Regangan Baja (et)", f"{epsilon_t:.4f}")
            pdf.add_result_row("Faktor Reduksi (phi)", f"{phi_fix:.3f}")
            pdf.add_result_row("Kondisi", status_str)
            pdf.ln(5)
            
            pdf.set_font('Arial', 'B', 11)
            pdf.cell(0, 8, "REKOMENDASI TULANGAN:", 0, 1)
            pdf.set_font('Arial', '', 11)
            pdf.cell(0, 8, f"- Luas Perlu (As): {As_req:.0f} mm2", 0, 1)
            pdf.cell(0, 8, f"- Tulangan Utama: {tulangan_utama}", 0, 1)
            pdf.cell(0, 8, f"- Kapasitas Momen: {Mu_capacity:.1f} kNm", 0, 1)
            
            pdf_bytes = pdf.output(dest='S').encode('latin-1')
            st.download_button(label="📥 DOWNLOAD LAPORAN BALOK", data=pdf_bytes, file_name=f"Desain_Balok_{b}x{h}.pdf", mime="application/pdf")
